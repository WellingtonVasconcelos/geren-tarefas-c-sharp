    ...
    public class TarefaController : BaseController
    {
        ...
        public IActionResult AtualizarTarefa([FromBody] Tarefa model, int idTarefa)
        {
            try
            {
                ...
                else if (string.IsNullOrEmpty(model.Nome) && !string.IsNullOrWhiteSpace(model.Nome)
                        && model.Nome.Count() < 4)
                    {
                        erros.Add("Favor informar um nome válido");
                    }
                ...
                if(string.IsNullOrWhiteSpace(model.Nome) && !string.IsNullOrWhiteSpace(model.Nome))
                {
